...
以后发布 `quanttide-handbook` 下的子手册，先看这个清单，不直接从聊天记录里找。

#### 待发布

当前无已整理但未发布的子手册。下一步从“第二批待审查”中选择目标，先审查内容成熟度，再决定是否整理为本地页。

#### 第二批待审查

| 子手册 | 所属目录 | 当前状态 | 下一步 |
|--------|----------|----------|--------|
| 团队管理手册 | 管理 | 第二批候选，尚未审查 | 先定位仓库和内容成熟度 |
| 项目管理手册 | 管理 | 第二批候选，尚未审查 | 先定位仓库和内容成熟度 |
| Django 手册 | 语言、框架和工具 | 第二批候选，尚未审查 | 先定位仓库和内容成熟度 |
| Figma 手册 | 语言、框架和工具 | 第二批候选，尚未审查 | 先定位仓库和内容成熟度 |

#### 第三批暂缓或待补

第三批主要是目录或骨架型子手册，默认暂不直接发布。除非负责人明确指定，否则先记录、后补内容、再审查。

| 子手册 | 所属目录 | 当前状态 | 下一步 |
|--------|----------|----------|--------|
| 企业服务手册 | 业务 | 第三批候选，暂缓发布 | 先定位仓库并确认是否有实质服务流程 |
| 开发者工具手册 | 研发 | 第三批候选，暂缓发布 | 先定位仓库并补齐工具使用场景 |
| 网络应用手册 | 研发 | 第三批候选，暂缓发布 | 先定位仓库并判断内容成熟度 |
| 数据分析手册 | 研发 | 第三批候选，暂缓发布 | 先定位仓库并判断是否只有目录 |
| 数字身份手册 | 研发 | 第三批候选，暂缓发布 | 先定位仓库并补齐身份场景和流程 |
| 文档管理手册 | 管理 | 第三批候选，暂缓发布 | 先定位仓库并判断是否有实质规范 |
| 客户支持手册 | 管理 | 第三批候选，暂缓发布 | 先定位仓库并补齐支持流程 |
| 法务管理手册 | 管理 | 第三批候选，暂缓发布 | 先定位仓库并确认合规边界 |
| 开源管理手册 | 管理 | 第三批候选，暂缓发布 | 先定位仓库并补齐开源治理流程 |
| 公共关系手册 | 管理 | 第三批候选，暂缓发布 | 先定位仓库并补齐对外沟通场景 |
| 新媒体管理手册 | 管理 | 第三批候选，暂缓发布 | 先定位仓库并补齐账号、内容和发布流程 |
| C 语言手册 | 语言、框架和工具 | 第三批候选，暂缓发布 | 先定位仓库并判断内容成熟度 |
| Docker 手册 | 语言、框架和工具 | 第三批候选，暂缓发布 | 先定位仓库并补齐镜像、构建和部署规范 |
| FastAPI 手册 | 语言、框架和工具 | 第三批候选，暂缓发布 | 先定位仓库并补齐服务开发流程 |
| FigJam 手册 | 语言、框架和工具 | 第三批候选，暂缓发布 | 先定位仓库并判断是否只有工具说明 |
| OpenAPI 手册 | 语言、框架和工具 | 第三批候选，暂缓发布 | 先定位仓库并补齐接口契约流程 |
| 高等教育手册 | 学科/行业 | 第三批候选，暂缓发布 | 先定位仓库并确认行业手册边界 |

#### 第四批暂停

| 子手册 | 所属目录 | 当前状态 | 下一步 |
|--------|----------|----------|--------|
| 客户端服务手册 | 业务 | 第四批暂停，之前判断为暂无有效内容或仓库状态异常 | 等有可用仓库或有效内容后再重新审查 |
...
- 工作手册整理时不要随便做保守泛化；原素材中已有明确平台、仓库、目录、命名例子、命令、流程动作或操作入口的，优先保留到草稿和发布页中。
